      ...
      <div className="bg-white p-5 rounded-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[25px]">
              {groupData?.name ? groupData?.name : "Loading..."}
            </h2>
          </div>
        ...
        <div className="mt-5 flex justify-between">
          <ul className="w-[49%] flex flex-col p-2 space-y-3 rounded-md border-[1px] border-slate-400">
            ...
            <Link
              className="inline-block text-[var(--clr-gold)] duration-300 hover:scale-[1.01]"
              to={`/major/${groupData?.stack.id}`}
            >
            ...
            >
              <MoreItem
                label="Ustoz"
                title={`${groupData?.mainTeachers[0].name} ${groupData?.mainTeachers[0].surname}`}
              />
            </Link>
            <Link
              className="inline-block text-[var(--clr-gold)] duration-300 hover:scale-[1.01]"
            ...
            >
              <MoreItem
                label="Yordamchi ustoz"
                title={`${groupData?.supportTeachers[0].name} ${groupData?.supportTeachers[0].surname}`}
              />
            </Link>
          </ul>
          <ul className="w-[49%] p-2 space-y-3 rounded-md border-[1px] border-slate-400">
            ...
            <MoreItem
              label="O'quvchi soni"
              title={groupData?.Students?.length}
            />
            <MoreItem
              label="Holati"
              title={groupData?.status ? "Faol" : "Faol emas"}
            />
            <MoreItem
              label="Yaratilgan vaqt"
              title={formatTime(groupData?.createdAt)}
            />
          </ul>
        </div>
      </div>
